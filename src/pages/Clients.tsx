...
const Clients = () => {
  ...
  const [form, setForm] = useState({ name: "", phone: "", license_plate: "" });
  ...
  const handleCreate = (e: React.FormEvent) => {
    e.preventDefault();
    createClient.mutate(
      { name: form.name, phone: form.phone || null, license_plate: form.license_plate },
      {
        onSuccess: () => {
          setDialogOpen(false);
          setForm({ name: "", phone: "", license_plate: "" });
        },
      }
    );
  };
      ...
      <div className="space-y-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="relative max-w-sm flex-1">
            ...
            <Input placeholder="Buscar por nombre, teléfono o matrícula..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9" />
          </div>
